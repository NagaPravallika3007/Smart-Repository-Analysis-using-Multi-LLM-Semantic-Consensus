...
def save_repo_cache(repo_url, cache_dir, index, document, file_type_count, file_names):
    ...
    # Also save the URL in a separate text file for easier access
    with open(os.path.join(cache_path, "repo_url.txt"), 'w') as f:
        f.write(repo_url)


def _read_repo_url_file(url_file):
    """Read a repo URL from a cache text file using tolerant encodings."""
    ...
    for encoding in encodings:
        try:
            ...
            if repo_url:
                repo_url = repo_url.lstrip("\ufeff").replace("\x00", "").strip()
...
def list_cached_repos(cache_dir):
    """Return a list of cached repository URLs from disk cache"""
    cached_urls = []
    ...
    for cache_folder in os.listdir(cache_dir):
        ...
        url_file = os.path.join(cache_path, "repo_url.txt")
        cache_file = os.path.join(cache_path, "cache_data.pkl")
        
        repo_url = None
        if os.path.exists(url_file):
            repo_url = _read_repo_url_file(url_file)
        
        if not repo_url and os.path.exists(cache_file):
            try:
                with open(cache_file, 'rb') as f:
                    cache_data = pickle.load(f)
                repo_url = cache_data.get('repo_url')
            except Exception:
                pass
        
        if repo_url:
            cached_urls.append(repo_url)
        else:
            cached_urls.append(f"Unknown URL (hash: {cache_folder})")
    return cached_urls
