...
def compute_consensus(responses: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    if len(valid_responses) == 0:
        error_details = "; ".join([f"{r['model_name']}: {r['response']}" for r in responses])
        return {
            "consensus_response": (
                "No valid responses received from LLM clients. "
                "Check that GROQ_API_KEY is configured in .env and restart the app. "
