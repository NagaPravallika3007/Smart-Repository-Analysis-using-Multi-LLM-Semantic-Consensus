...
from cache_manager import (
    get_repo_hash as cache_get_repo_hash,
    get_cache_path as cache_get_cache_path,
    is_repo_cached as cache_is_repo_cached,
    save_repo_cache as cache_save_repo_cache,
    load_repo_cache as cache_load_repo_cache,
    clear_old_cache as cache_clear_old_cache,
    list_cached_repos as cache_list_cached_repos,
)
from graph_utils import serialize_graph_data, deserialize_graph_data
...
# Cache helper wrappers for external tests and default cache handling

def get_repo_hash(repo_url):
    return cache_get_repo_hash(repo_url)


def get_cache_path(repo_url, cache_dir=CACHE_DIR):
    return cache_get_cache_path(repo_url, cache_dir)


def is_repo_cached(repo_url, cache_dir=CACHE_DIR):
    return cache_is_repo_cached(repo_url, cache_dir)


def save_repo_cache(repo_url, *args, **kwargs):
    # Support both existing internal signature and simplified test signature.
    if len(args) == 5 and isinstance(args[0], str):
        cache_dir, index, document, file_type_count, file_names = args
    elif len(args) == 4:
        cache_dir = kwargs.pop("cache_dir", CACHE_DIR)
        index, document, file_type_count, file_names = args
    elif all(k in kwargs for k in ["index", "document", "file_type_count", "file_names"]):
        cache_dir = kwargs.pop("cache_dir", CACHE_DIR)
        index = kwargs["index"]
        document = kwargs["document"]
        file_type_count = kwargs["file_type_count"]
        file_names = kwargs["file_names"]
    else:
        raise TypeError("save_repo_cache() requires either (repo_url, cache_dir, index, document, file_type_count, file_names) or (repo_url, index, document, file_type_count, file_names)")
    return cache_save_repo_cache(repo_url, cache_dir, index, document, file_type_count, file_names)


def load_repo_cache(repo_url, cache_dir=CACHE_DIR):
    return cache_load_repo_cache(repo_url, cache_dir)


def clear_old_cache(cache_dir=CACHE_DIR, max_age_hours=24):
    return cache_clear_old_cache(cache_dir, max_age_hours)


def safe_remove_temp_dir(path, retries=10, delay=0.5):
    import shutil
    import time
    import gc
    import os
    ...
    print(f"Could not delete temp dir: {path}")


def build_llm_clients():
    """Build a list of LLM client instances, skipping Groq clients when no API key is configured."""
    llm_clients = []
    if GROQ_API_KEY:
        llm_clients.extend([
        ...
        print("WARNING: GROQ_API_KEY is not set. Groq clients will be skipped.")
...
def process_repository_fresh(repo_url, repo_name):
    ...
    try:
        if clone_git_repo(repo_url, local_path):
            ...
            # Create LLM clients - Groq clients will be included only when GROQ_API_KEY is configured
            ...
            # Save to cache
            save_repo_cache(repo_url, CACHE_DIR, index, document, file_type_count, file_names)
        ...
        safe_remove_temp_dir(local_path)

def main():
    st.set_page_config(
        page_title="Smart Repository Analyzer",
        page_icon="📁",
        layout="wide",
    ...
    with st.sidebar:
        ...
        disk_cached_repos = cache_list_cached_repos(CACHE_DIR) if os.path.exists(CACHE_DIR) else []
        cache_count = len(disk_cached_repos)
        ...
        st.markdown(f"**📊 Cached repositories:** {cache_count}")

        #if st.session_state.cached_repos:
            #st.subheader("📦 In-memory cached repo URLs")
            #for repo in st.session_state.cached_repos:
                #st.write(repo)

        #if disk_cached_repos:
            #st.subheader("💾 Disk cached repository URLs")
            #for repo in disk_cached_repos:
                #st.write(repo)

        if not st.session_state.cached_repos and not disk_cached_repos:
            st.info("No repository cache is currently available.")
    ...
    with tab2:
        ...
        # Reload API keys from environment to ensure they're current
        ...
        if not groq_key:
            st.error(
                "🔑 GROQ_API_KEY is not configured. This app currently requires a valid Groq key in .env for real AI responses. "
                "Add `GROQ_API_KEY=your_key` to the .env file and restart the app."
            )
        else:
            st.success("✅ GROQ_API_KEY detected. Groq-based analysis is enabled.")

        if not hf_token:
            st.info(
                "ℹ️ HF_API_TOKEN is not configured. The HuggingFace client in this app is currently a placeholder and will not return a live model response."
            )
        if not gemini_key:
            st.info(
                "ℹ️ GEMINI_API_KEY is not configured. The Gemini client in this app is currently a placeholder and will not return a live model response."
            )
        ...
        if repo_url in st.session_state.cached_repos:
        ...
        elif is_repo_cached(repo_url, CACHE_DIR):
            ...
            if index is not None:
                # Create LLM clients for cached repo - Groq clients will be included only when GROQ_API_KEY is configured
                llm_clients = build_llm_clients()
